...
vs = 0.014;% vs: speed when slowed down (default v0/3)
...
% find points that are pair-unstable and cluster-unstable
...
prior_N2 = gmdistribution(paramCombis(sampleIndcs,:),bw.^2);
...
save(['../priors4D_M_' num2str(M) '_noVolExcl' ...
    ...
    '.mat'],'bw','cStabThresh','pStabThresh','supportLimits','prior_N2')
...
[~, AX, ~, hhist] = hplotmatrix(plotData,[],ones(size(plotData,1),1),supportLimits);
...
colormap(flipud(cmap_Oranges(1:24:end,:)))
for lineCtr = 1:numel(hhist)
    hhist(lineCtr).Color = plotColors(2,:); % make orange
end
%% export figure
