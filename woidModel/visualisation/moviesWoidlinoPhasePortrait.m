...
N = 40;
M = 18;
drdN_rev_values = 0.2%fliplr(0.4:0.2:1);
speed = [0.33];
slowspeed = [0.018];
...
k_dwell = 0.0036;
k_undwell = 1.1;
dkdN_dwell_values = 0.6%0:0.2:1;
dkdN_roam_values = 0.2
paramCombis = combvec(drdN_rev_values,dkdN_dwell_values,dkdN_roam_values);
...
for paramCtr = 1:nParamCombis % can be parfor but might impair movie quality
    ...
    dkdN_undwell = paramCombis(3,paramCtr);
    ...
    if exist([resultspath filename '.mat'],'file')...
        ...
        animateWoidTrajectories(out.xyarray,[moviepath filename],L,0.035,[],offset);%,out.param.rc);
